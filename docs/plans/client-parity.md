...
| Custom stats widgets | `stats.js`, `app.js`, `preferences.js`, `runner.js` | — | `/api/preferences` (`statsWidgets`) | **iOS pending** | Phase 4: user-defined widgets stored as a typed, server-validated JSON list (migration 037, `user_preferences.stats_widgets`). Closed enum schema, choreIds ownership-checked, title length-capped and rendered escaped, max 20 widgets / 4KB. Widgets are a presentation layer over existing stats endpoints (no new query surface). Wizard in the customize panel; sections keyed `widget:<uuid>`. iOS should decode the same list and render matching widgets |
